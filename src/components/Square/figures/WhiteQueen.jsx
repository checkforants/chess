...
const WhiteQueen = ({colorNow, setColorNow, figurePositions,squareClicked, setSqureClicked, possibleMoves, setPossibleMoves, fullName,rowName,name,...props}) => {
	...
	const handleClick = () =>{
		...
		if (fullName===squareClicked){
			...
			while (t_edge<9 && figurePositions[name+String(t_edge)][0]!=='W'){

				arr.push(name+String(t_edge))
				if (figurePositions[name+String(t_edge)][0]==='B'){break}
				t_edge+=1
			}
			while (b_edge>0 && figurePositions[name+String(b_edge)][0]!=='W'){
				arr.push(name+String(b_edge))
				if (figurePositions[name+String(b_edge)][0]==='B'){break}
				b_edge-=1
			}
			while (r_edge<8 && figurePositions[str[r_edge]+rowName][0]!=='W'){
				arr.push(str[r_edge]+rowName)
				if (figurePositions[str[r_edge]+rowName][0]==='B'){break}
				r_edge+=1
			}
			while (l_edge>-1 && figurePositions[str[l_edge]+rowName][0]!=='W'){
				arr.push(str[l_edge]+rowName)
				if (figurePositions[str[l_edge]+rowName][0]==='B'){break}
				l_edge-=1
			}
	...
	return (
		...
		onClick={(e)=>handleClick()}>
			<img {...props} src={require('../whitePieces/Queen.png')}>
